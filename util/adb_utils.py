...
if "ANDROID_HOME" in os.environ:
    if system == "Windows":
        command = os.path.join(os.environ["ANDROID_HOME"], "adb.exe")
    else:
        command = os.path.join(os.environ["ANDROID_HOME"], "adb")
else:
...
class AdbUtils(object):
    ...
    # connect命令
    def connect(self):
        cmd = "%s connect %s" % (command, self.device_id)
        return subprocess.run(cmd, shell=True, capture_output=True)

    # adb命令
    def sync_adb(self, args):
    ...
    def getFocusedPackageAndActivity(self):
        ...
        out = self.shell("dumpsys window w | %s \/ | %s name=" % (find_util, find_util)).stdout.read().decode('utf-8')
    ...
    def getBatteryLevel(self):
        ...
        获取电池电量
        """
        level = self.shell("dumpsys battery | %s level" % find_util).stdout.read().decode('utf-8').split(": ")[-1]
    ...
    def getScreenResolution(self):
        ...
        """
        pattern = re.compile(r"\d+")
        out = self.shell("dumpsys display | %s PhysicalDisplayInfo" % find_util).stdout.read().decode('utf-8')
    ...
    def unlockPhone(self):
        ...
        def _swipUnlock():
            ...
            self.shell("sendevent /dev/input/event1 0 57 0")
